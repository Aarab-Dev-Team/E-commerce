        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-ghost" style="padding: 6px 16px; font-size: 12px;" onclick="openOrderModal({{ $order->id }})">
                            View
                        </button>
                    </td>
                </tr>
...
<script>
    ...
    function populateOrderModal(order) {
        ...
        const statusBadge = document.querySelector('#orderModal .badge');
        statusBadge.className = `badge badge-${order.status}`;
        statusBadge.innerText = order.status;

        // Set date
        document.getElementById('modal-order-date').innerText = `Placed on ${new Date(order.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`;
        ...
        // Shipping address (parse from text)
        document.getElementById('modal-shipping-address').innerHTML = order.shipping_address.replace(/\n/g, '<br>');
        ...
        else if (order.payment_method === 'paypal') paymentMethod = 'PayPal';
